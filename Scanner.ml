...
let lexical_analysis source =
  ...
  let make_number start =
    let rec get_position ennd =
      ...
      | '0'..'9' -> get_position (ennd + 1)
      | _ -> ennd
      in let ende = get_position start
      in (Stdlib.int_of_string(String.sub source start (ende - start))
         , ende) in
  ...
  let rec analyze current_pos token_list=
    ...
    | '/' -> if (matches (current_pos + 1) '/') then
        analyze (skip_line (current_pos + 1))
          token_list
      ...
      if (bl, next_pos) = (false, -1)
        then failwith "Error unterminated string"
            ...
            (STRING(make_string (current_pos + 1) (next_pos -1))
             ::token_list)
    | '0'..'9' -> let (num, nxt_pos) =
                    make_number current_pos in
      analyze nxt_pos ((INTEGER(num))::token_list)
    | 'a'..'z' | 'A'..'Z' | '_' ->
      let (ident, nxt_pos) = create_identifier current_pos in
      match ident with
      | "let" -> let tok = LET in analyze nxt_pos (tok::token_list)
      ...
      | _ ->
        let tok = IDENTIFIER(ident) in analyze nxt_pos (tok::token_list)
